...
namespace Validadores {
    ...
    void definirFechaInicio(const char *fecha) { std::strcpy(fechaI, fecha); }
    ...
    // Convierte fecha YYYY-MM-DD a valores numéricos. Asume un formato válido.
    bool parsearFecha(const char *fecha, int &anio, int &mes, int &dia) {
        if (charVacio(fecha) || !TamañoValido(fecha, 10) || fecha[4] != '-' || fecha[7] != '-') {
            return false;
        }
    ...
    int compararFechas(const char *fechaA, const char *fechaB) {
        int anioA, mesA, diaA;
        int anioB, mesB, diaB;

        if (!parsearFecha(fechaA, anioA, mesA, diaA) || !parsearFecha(fechaB, anioB, mesB, diaB)) {
            return 0;
        }
    ...
    bool fechaRegistraAntesDelTorneo(const char *fechaRegistro, const char *fechaInicio, char *mensajeError) {
        if (compararFechas(fechaRegistro, fechaInicio) >= 0) {
            std::strcpy(mensajeError, "La fecha de registro debe ser anterior a la fecha de inicio del torneo.");
            return false;
        }
        return true;
    }

    bool registroDentroDeUnAnoPrevio(const char *fechaRegistro, const char *fechaInicio, char *mensajeError) {
        int anioRegistro, mesRegistro, diaRegistro;
        int anioInicio, mesInicio, diaInicio;

        if (!parsearFecha(fechaRegistro, anioRegistro, mesRegistro, diaRegistro) || !parsearFecha(fechaInicio, anioInicio, mesInicio, diaInicio)) {
            std::strcpy(mensajeError, "Error interno en el cálculo de fechas.");
            return false;
        }

        if (anioInicio - anioRegistro > 1) {
            std::strcpy(mensajeError, "La fecha de registro no puede ser más de un año anterior al inicio del torneo.");
            return false;
        }
        ...
        // Si es exactamente un año antes, entonces mes/dia deben ser iguales o anteriores.
        if (mesRegistro < mesInicio) {
            return true;
        }
        if (mesRegistro > mesInicio) {
            std::strcpy(mensajeError, "La fecha de registro no puede ser más de un año anterior al inicio del torneo.");
    ...
    bool fechaPartidoDentroDeTorneo(const char *fechaPartido, const char *fechaInicio, const char *fechaFin, char *mensajeError) {
        if (compararFechas(fechaPartido, fechaInicio) < 0) {
            std::strcpy(mensajeError, "La fecha del partido no puede ser anterior a la fecha de inicio del torneo.");
            return false;
        }
        if (compararFechas(fechaPartido, fechaFin) > 0) {
            std::strcpy(mensajeError, "La fecha del partido no puede ser posterior a la fecha de fin del torneo.");
            return false;
        }
        return true;
    }

    bool FechaFinMayorInicio(const char *fechaInicio, const char *fechaFin, char *mensajeError) {
        if (compararFechas(fechaFin, fechaInicio) < 0) {
            std::strcpy(mensajeError, "La fecha de finalización no puede ser anterior a la fecha de inicio.");
            return false;
        }
        return true;
    }
    ...
    bool CompararFechas(const char *fecha, char *mensajeError) {}
    ...
    namespace menu {
        ...
        void datosInicialesTorneo(SistemaDeportivo *MiSistema) {
            ...
            do {
                ...
                if (Validadores::FechaFinMayorInicio(torneoAux.fechaInicio, torneoAux.fechaFin, mensajeError)) {
                    fechaValida = true;
                } else {
